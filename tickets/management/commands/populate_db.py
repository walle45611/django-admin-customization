...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for i in range(10):
            ...
            start_time = datetime.now(pytz.utc) + timedelta(days=random.randint(1, 365))
            start_time_naive = make_naive(start_time)

            concert = Concert.objects.create(
                name=f"{random.choice(concert_prefix)} {category.name} {random.choice(concert_suffix)}",
                description="",
                venue=venue,
                starts_at=start_time_naive,  # 使用沒有時區資訊的日期時間值
        ...
        for i in range(500):
            ...
            paid_at_time = datetime.now(pytz.utc) - timedelta(days=random.randint(1, 365))
            paid_at_time_naive = make_naive(paid_at_time)
            ...
            Ticket.objects.create(
                ...
                paid_at=paid_at_time_naive,  # 使用沒有時區資訊的日期時間值
